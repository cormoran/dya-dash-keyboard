...
- **FFC ケーブルの取り扱い**：表と裏に注意し、丁寧に扱ってください。
  ...
  - 予備を同梱していますが、予備も含めて FFC ケーブルが断線してしまったと思われる場合はサポートしますのでご連絡お願いします。
...
- （おまけとして同梱されている場合）白い PE スイッチパッド（下の写真に写っていません）
...
1. （おまけとして同梱されている場合 & 敷きたい場合）トップケースに PE スイッチパッドをしく

   - PE スイッチパッドはおまけとして同梱されている場合と同梱されていない場合があります。
   - PE スイッチパッドは矢印キーなしモデルの形状でカットされています。 - トラックボールをつける側では不要な部分を切り取って敷いてください。 - 矢印キーありモデルの右では矢印に当たるところも切り取って敷いてください。
   - スイッチパッドを敷くと打鍵音が大きくなる印象があるので、静音スイッチを使用する場合は敷かない方が良いかもしれません。

1. 基板に電源スイッチカバーをつける

   - 電源スイッチカバーを回路基板の電源スイッチにはめ込みます
   - **電源スイッチカバーの突起部分が基板の裏側寄りになる向きではめてください**
   - 電源スイッチカバーの側面は非常に薄く、組み立て時に裂けて外れてしまうことがあります。が、側面は機能上必要ではなく、外れてしまっても機能/組み立て後の見た目ともに問題なく動作します
...
以下のスイッチパッドは同梱されている場合とされていない場合があります。
![スイッチパッド](img/switch_pad.jpeg)
...
矢印キーありモデルでは、左側で使用したものに加えて以下の部品を使います。
...
矢印キーなしモデルの場合は矢印キー関連部分は適宜読み飛ばしてください。
...
3. **トラックボールケースに基板を取り付け**
   ...
   - バージョン v2.1.1 以降の場合:
     - エレコム/ぺリックス赤玉用と染色 POM 球用のアジャスターが同梱されています。
       ![センサー設置](img/sensor_new1.jpeg)
     - 下の "エレコム赤玉用の取り付け方法" か "染色 POM 球用の取り付け方法" に従ってください。
     - フロストアクリル球（紫）も"染色 POM 球用の取り付け方法"で動作することを確認しています。
   - バージョン v2.1 以前の場合
     - 下の "バージョン v2.1 以前の場合" に従ってください。
   - エレコム/ぺリックス赤玉用の取り付け方法
     1. 板状の方のパーツを用意します。
        ![センサー設置](img/sensor_new9.jpeg)
     2. センサーをケースに差し込み、裏側(レンズと逆側)から板状のアジャスターを差し込みます。
        FFC コネクター部分に力がかからないように注意してください。
        ![センサー設置](img/sensor_new10.jpeg)
        ![センサー設置](img/sensor_new11.jpeg)
     3. 奥までセンサーとアジャスターを両方奥まで差し込みます。
        アジャスターが出っ張っているとボトムケースと干渉するので奥まで差し込んでください。
        ![センサー設置](img/sensor_new12.jpeg)
     4. レンズがずれている場合は少し調整します。
        （ケースの窓とレンズの位置は完全には一致せず、少しだけ左右にずれた状態にはなります）
        ![センサー設置](img/sensor_new13.jpeg)
   - 染色 POM 球用の取り付け方法

     - ゆとりが少ないので難易度が少し高いかもしれないです

     1. センサーのレンズに画像のようにアジャスターはめ込みます
        センサーの左右の厚みとアジャスターの左右の厚みが同じになる向きで設定してください。
        ![センサー設置](img/sensor_new2.jpeg)
        ![センサー設置](img/sensor_new3.jpeg)
     2. アジャスターがずれないようにしながらトラックボールケースにセンサーを差し込みます。
        取り付け後にセンサーがずれないようにゆとりがない設計になっています。少し力を入れて差し込む必要があります。
        FFC コネクター部分に力がかからないように注意してください。
        ![センサー設置](img/sensor_new4.jpeg)
     3. 奥まではめ込みます。奥まで挿さらない場合はアジャスターがずれている可能性があります。
        ![センサー設置](img/sensor_new5.jpeg)
        ![センサー設置](img/sensor_new6.jpeg)
        ![センサー設置](img/sensor_new7.jpeg)
     4. レンズが大きくずれている場合は少し調整します。
        （ケースの窓とレンズの位置は完全には一致せず、少しだけ左右にずれた状態にはなります）
        ![センサー設置](img/sensor_new8.jpeg)

   - 取り替える場合の外し方

     - ゆとりが小さいため取り外しは少し難しいです
     - レンズを傷つけないように注意しながら、ケースの窓から先の細いペンチ等でレンズの縁部分を押し下げることで取り外すことができます。
       レンズの球部分を傷つけないように注意してください。
       ![センサー設置](img/sensor_new14.jpeg)

   - バージョン v2.1 以前の場合
     1. トラックボールケースにセンサーをはめ込みます
     2. センサーのレンズ部分がボール側になるような向きで差し込みます
     3. 差し込んだ後、奥まで押し込みます
     4. この時、FFC ケーブルに力を加えると外れたりずれてしまうので注意してください
