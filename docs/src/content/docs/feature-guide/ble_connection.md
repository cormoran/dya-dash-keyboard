...
V2, V3 でデフォルトのキーマップが違います。説明は V3 向けに書き足しましたが、画像は V2 のもののままになっています。
...
- V2: 左手の設定ボタン右で４つの無線接続先を切り替えられ、右手の LED に接続状態が表示されます。
- V3: 左手の設定ボタンで４つの無線接続先を切り替えられます
  - 左手設定ボタン左: 現在の接続先番号-1に接続 (4->3->2->1->4->...)
  - 左手設定ボタン右: 現在の接続先番号+1に接続 (1->2->3->4->1->...)
...
1. 左手設定ボタン(右)で解除したい接続先を選択
2. 現在選択している接続先とのペアリングを解除
   - V2: 左手左下のキー + 左手設定ボタン右
   - V3: 左手の設定ボタンどちらかを５秒以上長押し
...
- 左手左下+g: 一定時間 LED が点灯して 右側の LED にペアリング状態を表示します
- 電源を入れ直す: 電源を入れ直すとバッテリー残量表示後に接続先が表示されます
- USB 給電: 右手に USB で給電すると LED が常点灯状態になって接続先が表示されます

## USB 接続

右手を PC と USB 接続した状態で以下のボタンを押すと USB/BLE 優先モードを切り替えられます。
