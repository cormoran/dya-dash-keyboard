...
V3 では DYA2 と同じ仕組みでキーキャップの裏側に取り付けられるタッチパッドを導入したためあまりのポートはなくなりました。

V2 でもパッドから配線を伸ばしてうまくキーキャップの裏側に取り付けられるタッチパッドを固定できれば同様にタッチパッドを追加できるかもしれません。
...
V3 では左右の有線接続が実装済みです。PC と USB 有線通信をしている時に下側の左右の USB ポートどうしを接続することで、左右の BLE 接続が解除されて自動的に有線通信に切り替わります。

### V2

トラックボールセンサーのモーション入力とキーボードの左右有線通信を共用するような回路設計をしているのでうまくプログラムを書けば有線通信が実装できます。

- V1 基板では抵抗値の選定ミスで有線通信ができなさそうな状態です。
- V2 基板では多分できる気がする状態に修正しましたが、まだ動作未確認でできるかどうかわからない状態です。

2026/04 現在、実装予定はありませんが引き続き実装に興味はあるので実装される可能性は残っています。
