...
- 左下のキー or 親指周りのキーを順に２個同時押しの状態（default->Left/right SPACE->Config とレイヤー遷移）で、推している間だけ config layer に入ります。
...
- soft off は試したのですが左でトリガーすると上手く復帰しないので無効にしてあります（TODO: マッピングを後で消す）
